...
def _sample_allowlist(
    ...
    *,
    ...
    cosign_cert: str = (
        "https://github.com/veriker/veriker"
        "/.github/workflows/release.yml@refs/tags/v0.3.0"
    ),
    registry_org: str = "ghcr.io/veriker/",
    oci_artifact: str = (
        "ghcr.io/veriker/veriker-plugins/spec-sha-pin:v0.3.0"
    ),
) -> dict:
...
def test_plugin_registry_org_not_allowlisted_rejected(tmp_path: Path):
    ...
    plugin_name = "spec_sha_pin"
    # Allowlist's registry_org_allowlist is ghcr.io/veriker/ but the entry
