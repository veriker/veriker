...
``BundleVerifier.verify()``) and the file-integrity walks now derive ownership
from ``classify_path``; the complement skip-sets enumerated above were rewired
to this single source (the Pass-3 surplus sweep, strict-SHA membership, and
both ``_SEAM_EXCLUDED`` DSSE literals no longer carry their own copies). The
parity harness (``tests/test_integrity_ownership_parity.py``) pins this map ≡
the as-built walks over the real ``examples/`` corpus.
